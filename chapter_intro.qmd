...
tsla_returns <- na.omit(tsla_returns)  # Remove NAs

# Compute mean and standard deviation of returns
mean_return <- mean(tsla_returns)
std_dev <- sd(tsla_returns)

# Get actual risk-free rate (e.g., 3-month Treasury Bill rate)
getSymbols("^IRX", src = "yahoo", from = Sys.Date() - 30, to = Sys.Date(), auto.assign = TRUE)
risk_free_rate <- as.numeric(last(Cl(IRX))) / 100
...
- In a special case when $R \sim \phi(\mu,\sigma)$, we can find $X$ using the Excel function  `norm.inv()`.
    - For given $1-p$, `norm.inv(1-p,$\mu, \sigma$)` is $X$ that satisfies $\text{Prob}\left(R \leq X \right) = 1-p$.
        - VaR at 5\% =  `norm.inv(0.05,0,1) = -1.645` (R: `qnorm(0.05, mean = 0, sd = 1)`)
        - VaR at 10\% =  `norm.inv(0.1,0,1) = -1.282` (R: `qnorm(0.10, mean = 0, sd = 1)`)

    
:::{.aside}    
Closed-form: $VaR(X) = \Phi ^{-1}(1-p)\sigma + \mu$
:::
...
getSymbols("PLTR", src = "yahoo", from = "2020-01-01", to = Sys.Date(), auto.assign = TRUE)

# Compute daily returns (convert to numeric)
pltr_returns <- dailyReturn(Cl(PLTR))
pltr_returns <- na.omit(pltr_returns)  # Remove NAs
pltr_returns_numeric <- as.numeric(pltr_returns)  # Ensure numeric format

# Compute VaR and Expected Shortfall
confidence_level <- 0.05
VaR_95 <- VaR(pltr_returns_numeric, p = confidence_level, method = "historical")
ES_95 <- ES(pltr_returns_numeric, p = confidence_level, method = "historical")

# Print risk measures
print(paste("95% Historical VaR:", round(VaR_95, 4)))
print(paste("95% Expected Shortfall:", round(ES_95, 4)))

# Visualize Return Distribution with VaR Line
chart.Histogram(pltr_returns, 
                breaks = 50,
                main = "Palantir (PLTR) Return Distribution with VaR & ES",
                xlab = "Daily Returns",
